...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_range = 0
        while(checkPlace != beehive):
            if(random_or_none(checkPlace.bees) != None and current_range >= self.min_range and current_range <= self.max_range):
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while curr_place.exit:
                ...
                if ant and ant not in self.have_doubled_ants:
                    ant.damage = 2 * ant.damage
                    self.have_doubled_ants.append(ant)

                # if the ant is under bodyguard ant
                if isinstance(ant, BodyguardAnt) and ant.contained_ant and ant.contained_ant not in self.have_doubled_ants:
                    ant.contained_ant.damage = 2 * ant.contained_ant.damage
                    self.have_doubled_ants.append(ant.contained_ant)
